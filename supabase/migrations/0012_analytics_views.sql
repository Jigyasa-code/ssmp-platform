...
select
  ...
  count(t.id) filter (where t.status = 'Open')                          as open_queries,
  count(t.id) filter (where t.status = 'In Progress')                   as in_progress_queries,
  count(t.id) filter (where t.status = 'Resolved')                      as resolved_queries,
  count(t.id) filter (where t.category = 'Academic')                    as academic_queries,
  count(t.id) filter (where t.category = 'ERP/Tech')                    as erp_tech_queries,
  count(t.id) filter (where t.category = 'Infrastructure')              as infrastructure_queries,
...
left join public.support_queries t on t.mentor_id = f.id
where f.role = 'faculty'
group by f.id;

comment on view public.faculty_performance_summary is
  'Per-faculty query metrics. Powers Feature 4 (faculty self-report) and the HOD leaderboard.';

-- ── Per-student summary (drives Feature 5) ───────────────────────────
drop view if exists public.student_query_summary cascade;
create view public.student_query_summary
with (security_invoker = true) as
select
  s.id                                                        as student_id,
...
left join public.support_queries t on t.student_id = s.id
where s.role = 'student'
group by s.id;

-- ── Daily query trend (sparklines on the HOD + faculty dashboards) ───
drop view if exists public.query_daily_trend cascade;
create view public.query_daily_trend
with (security_invoker = true) as
select
  t.mentor_id,
  date_trunc('day', t.created_at)::date                          as day,
  count(*)                                                       as queries_created,
  count(*) filter (where t.status = 'Resolved')                  as queries_resolved,
