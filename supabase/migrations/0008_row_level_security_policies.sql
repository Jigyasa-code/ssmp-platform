...
--   policy allows (system messages, notifications, query state machine).
...
drop policy if exists queries_update_mentor       on public.support_queries;
...
-- A student may only open a query for themselves, and only against the
...
-- No DELETE policy: queries are an audit record and are never deleted.


-- =====================================================================
-- query_messages
-- =====================================================================
drop policy if exists messages_select_participants on public.query_messages;
...
-- public.post_query_message(), which is SECURITY DEFINER and re-checks
...
grant select, insert, update         on public.support_queries         to authenticated;
grant select                         on public.query_messages         to authenticated;
...
grant usage                          on sequence public.query_code_seq to authenticated;
