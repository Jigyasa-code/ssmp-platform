...
-- ---------------------------------------------------------------------
-- Postgres has no DROP VALUE for an enum, and support_queries rows
...
-- existing query keeps the category it was raised with.
--
...
-- old one would silently relabel historical queries.
-- =====================================================================

alter type public.query_category add value if not exists 'Academics';
alter type public.query_category add value if not exists 'Examination';
alter type public.query_category add value if not exists 'Behavioural';
alter type public.query_category add value if not exists 'Administrative';
alter type public.query_category add value if not exists 'Others';
