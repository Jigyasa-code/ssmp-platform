 ...
 * Answering "No" reopens the query and notifies the mentor; the faculty
...
export default function ResolutionConfirmation({ query, onDone }) {
  const { run, pending } = useAsyncAction();
  const [comment, setComment] = useState('');
  const [choice, setChoice] = useState(null);

  if (query.resolution_status !== 'pending_confirmation') return null;
  ...
  const rejectionsUsed = query.reopen_count ?? 0;
  ...
  const respond = (response) =>
    run(
      async () => {
        const { error } = await supabase.rpc('confirm_query_resolution', {
          p_query_id: query.id,
          p_response: response,
      ...
      },
      {
        successMessage:
          response === 'yes'
            ? 'Thank you — this query is now closed.'
            : 'Query reopened. Your mentor has been notified.',
        onSuccess: onDone
      }
    );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-body-sm text-on-surface-variant">
            Your mentor has marked <strong>{query.subject}</strong> as resolved. Please confirm so we can
          ...
          {rejectionsUsed > 0 && canReject && (
            <p className="mt-2 text-label-sm text-on-warning-container">
              You have reopened this query {rejectionsUsed} time{rejectionsUsed > 1 ? 's' : ''}. You can do
          ...
          {!canReject && (
            <p className="mt-2 rounded bg-warning-container/60 px-3 py-2 text-body-sm text-on-warning-container">
              You have already reopened this query {MAX_REJECTIONS} times, which is the maximum. If it is
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {choice !== 'yes' && canReject && (
              ...
              >
                ...
                {choice === 'no' ? 'Reopen query' : 'No, still an issue'}
              </button>
            )}
            {choice && (
